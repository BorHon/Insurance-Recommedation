...
def produce_train():
    ...
    age = fake.random_int(min=8, max=75)
    specific = fake.random_int(min=0, max=100000) % 37 == 1
    Family_insured = fake.random_int(min=0, max=100000) % 2 == 0
    if age <= 18 :
        income = 0
        marriage = "unmarried"
    else:
        income = fake.random_int(min=0, max=100000)
    ...
    BMI = weight / (height ** 2)
    insurance = ""
    deposit = income * fake.random_int(min=int(age/10), max=int(age/10 + 30))
    if age < 12:
    ...
    if insurance=="":
        if deposit <3*income and BMI<23.9 and BMI>18.5:
            insurance="E"
        else:
            insurance="D"
